...
export default function randomColor(options) {
  ...
  if (options.count !== null && options.count !== undefined) {
    ...
    options.count = null;
    options.totalColors = totalColors;
...
function pickHue(options) {
  if (colorRanges.length > 0) {
    ...
    if (typeof options.hue === "object") {
      if (options.hueRange === undefined) {
        options.hueRange = hueRange;
      } else {
        hueRange = options.hueRange;
      }
      var hue =
        hueRange[0] +
        colorIndex * ((hueRange[1] - hueRange[0]) / options.totalColors);
      colorIndex += 1;
    } else {
...
function pickSaturation(hue, options) {
  ...
  if (typeof options.hue === "object") {
    if (options.S === undefined) {
      cOptions = { ...options };
      cOptions.hue = "";
      options.S = pickSaturation(hue, cOptions);
    }
    return options.S;
  }
...
function getRealHueRange(colorHue) {
  if (typeof colorHue === "object") {
    let hue1 = getRealHueRange(colorHue[0]);
    let hue2 = getRealHueRange(colorHue[1]);
    let h1 = (hue1[0] + hue1[1]) / 2;
    let h2 = (hue2[0] + hue2[1]) / 2;
    return [h1, h2];
  } else if (!isNaN(colorHue)) {
