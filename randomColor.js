...
export default function randomColor(options) {
  ...
  if (options.count !== null && options.count !== undefined) {
    ...
    options.count = null;
    var colorDict = new Object();
    if (Array.isArray(options.hue)) {
      //totalColors = totalColors * options.hue.length;
      var hueList = options.hue;
      for (let i = 0; i < hueList.length; i++) {
        options.hue = hueList[i];
        colorDict[options.hue] = []
        for (let j = 0; j < totalColors; j++) {
          ...
          colors.push(color);
          colorDict[options.hue].push(color)
        }
      }
      return colorDict;
    }
